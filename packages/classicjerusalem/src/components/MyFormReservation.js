...
const ReservationForm = ({state, actions, libraries, propertyName, propertyCode, setSuccessFirstForm, category}) => {
  ...
  const [propertyID, setPropertyID] = useState(propertyCode+'-'+propertyName)
  ...
  const validateForm = () => {
    ...
    console.log("form in validate: ", form.current)
    ...
    if (!numOfPeople.trim()) {
      formErrors.numOfPeople = 'Please enter the number of people.';
    }
    ...
    if (!arrivalDate) {
      ...
      } else if (arrivalDate >= checkoutDate) {
        formErrors.arrivalDate =
          'The arrival date must be earlier than the checkout date.';
    }


    if (!checkoutDate) {
        formErrors.checkoutDate = 'Please select the checkout date.';
      } else if (checkoutDate < currentDate) {
        formErrors.checkoutDate = 'Please select a future checkout date.';
      } else if (arrivalDate >= checkoutDate) {
        formErrors.checkoutDate ='The checkout date must be later than the arrival date.';
      } else if((category === 10 || category === 11) && (new Date(checkoutDate).getTime() - new Date(arrivalDate).getTime() < 31536000000)){
        formErrors.checkoutDate =
        'This is a long term property, the length of stay should be 1 year or more.';
      }
      ...
      <form ref={form} onSubmit={handleSubmit}>
        ...
        <Label>
          <Input
            ...
            required
          />
          {errors.numOfPeople && <Error>{errors.numOfPeople}</Error>}
        </Label>
        ...
        <Label>
          ...
          <DateTimePicker
            ...
            required
          />
          {errors.checkoutDate && <Error>{errors.checkoutDate}</Error>}
        </Label>



        <Button type="submit" value="Send">Book this Aparment</Button>
