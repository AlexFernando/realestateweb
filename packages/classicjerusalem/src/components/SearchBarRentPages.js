...
const SearchBarRentPages = ({state, actions, libraries, active, allRentProperties, handleResults, setSearchTerm, setArrResult}) => {
    ...
    const handleSearch = () => {
        ...
        //saving search termS to show on the UI

        // Perform search based on the selected filters
        const filteredProperties = allRentProperties.filter(property => {
    ...
    return ( 
        <SearchBarClearResult>
            ...
            {
                buttonClear?
                <ButtonClear onClick={handleClearResult}> Clear All </ButtonClear>
                : null
            }
...
const SearchBarClearResult = styled.div`
    display: flex;
...
export default connect(SearchBarRentPages);
